...
<%
	...
	MusicDAO dao3 = new MusicDAO();
	

	ArrayList<Music> mulist = new ArrayList<Music>();
	ArrayList<Music> memlist = new ArrayList<Music>();
	if(mem!=null){
		mulist = dao3.totmusicList();
		memlist = dao3.mem_music(mem_no);
	}
	...
	if(!price.equals("")){ // equlas로 비교하기 위해 iprice를 String형 price로 받았다.
		...
		if(mem.getMem_cong() >= p){ // 현재 세션에 연결된 mem의 정보중 getMem_cong을 불러와 p(가격)과 비교한다.
			...
			for(int i= 0; i<memlist.size();i++){
				fullitem += memlist.get(i).getMusic_title();
			}
			if(!fullitem.contains(iname)){
				...
				dao2.addMemberMusic(mem_no,ino); // dao2.addMemberitem(mem.getMem_no(),ino);가 True이면 알아서 만들어놓은 모듈(함수) sql이 동작된다.
				dao.usecong(mem_no,p,"배경음악");
...
<div style="height:1000px;width:250px;padding-top:50px;float:left" id="mySidebar">
  ...
  <div class="w3-large" style="font-weight:bold">
    ...
    <div id="demoAcc1" class="w3-bar-block w3-hide w3-padding-large w3-medium" >
      <a href="Main03_itemes.jsp" onclick="w3_close()" class="w3-bar-item w3-button" >
  ...
  <div class="w3-large" style="font-weight:bold">
    ...
    <div id="demoAcc4" class="w3-bar-block w3-hide w3-padding-large w3-medium">
      ...
      <a href="Main03_itemes_Music3.jsp" class="w3-bar-item w3-button " style="background-color:#ccc"><i class="fa fa-caret-right w3-margin-right"></i>Jazz</a>
    </div>
  </div>
 </div>
...
<div style="float:left; padding-left:200px">
  <div>
	  <br><br>
	    <p style="font-weight:bold;font-size:25px;padding-right:150px" align="center">Jazz 목록</p>
		<table style="padding-left:50p" width="1300px" border;>
			<tr>
		<%for(Music i:mulist) {
		if(i.getMusic_genre().equals("재즈")){%>
			<form name="buy<%=i.getMusic_title()%>">
			<td><br><p>제목 : <%=i.getMusic_title()%> / 장르 : <%=i.getMusic_genre() %><br><b><%=i.getMusic_price() %>콩알</b></p>
			<button class="w3-button w3-black" id="buy<%=i.getMusic_title()%>">Buy now <i class="fa fa-shopping-cart"></i></button>
			</td>
			<input type="hidden" value="<%=i.getMusic_no()%>" name="ino"/>
			<input type="hidden" value="<%=i.getMusic_price()%>" name="iprice"/>
			<input type="hidden" value="<%=i.getMusic_title()%>" name="iname"/>
		</form>
			<%}} %>
			</tr>
		</table>  
...
window.onload=function(){
	...
	<%for(Music i:mulist) {
		if(i.getMusic_genre().equals("재즈")){%>
		var buy<%=i.getMusic_title()%> = document.querySelector("#buy<%=i.getMusic_title()%>");
		buy<%=i.getMusic_title()%>.onclick = function(){
			document.querySelector("form[name=buy<%=i.getMusic_title()%>]").submit();
		}
		<%}} %>
};
function myAccFunc1() {
