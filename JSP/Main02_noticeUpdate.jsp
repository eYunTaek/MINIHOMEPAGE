...
<%
	...
	boolean isEnd = false;
	if(proc.equals("upt")){
		Notice upt = new Notice(notice_no, title, content);
		isEnd = dao.updateNotice(upt);
	}
	Members mem = null;
	boolean isLog = false;
...
<body>
	...
	<div id="notice">
		...
		<table>
			...
			<tr>
				<th>제목</th><td><input type="text" name="title" value="<%=nt.getNotice_title() %>" /></td>
			</tr>
			<tr>
				<th>내용</th><td><textarea name="content" rows="10"><%=nt.getNotice_content() %></textarea></td>
			</tr>
			<tr>
		...
		<div align="center">
			...
			<input type="button" class="btn" value="취소" onclick="location.href='Main02_noticeView.jsp?notice_no=<%=nt.getNotice_no() %>'" />
		</div>
		</form>
	</div>
	<br><br><br>
	...
	if(isEnd){
		alert("수정 완료했습니다.");
		location.href = "Main02_noticeView.jsp?notice_no="+no;
	}
	var isLog = <%=isLog%>;
	if(isLog){
